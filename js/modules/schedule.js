...
function renderWeeklyView(schedule, conflicts) {
  ...
  DAYS.forEach(day => {
    ...
    if (!events.length) {
      ...
      events.forEach(e => {
        ...
        html += `
          <div class="cal__event ${color}" data-id="${id}" role="button" tabindex="0">
  ...
  html += '</div></div>';
  html += '<div class="cal-detail-panel" id="cal-detail-panel"></div>';
  return html;
}
    ...
    <div class="card">
      ...
      <div class="daily-schedule">
        ${todayClasses.length ? todayClasses.map(c => {
          const id = c._id || c.id;
          return `
          <div class="daily-class ${conflicts.has(id) ? 'conflict' : ''}">
            ...
            <div class="daily-class__info">
              ...
              <p>${c.lecturer ? escapeHtml(c.lecturer) + ' · ' : ''}${formatLocation(c)}</p>
            </div>
            <div class="schedule-item-actions">
              <button type="button" class="btn btn-ghost btn-sm edit-schedule" data-id="${id}">Ubah</button>
              <button type="button" class="btn btn-ghost btn-sm delete-schedule" data-id="${id}" style="color:var(--color-danger)">Hapus</button>
              <button type="button" class="btn-gcal add-gcal" data-id="${id}" title="Google Calendar">📅</button>
            </div>
          </div>
...
function showDetailModal(id) {
  ...
  modal.querySelector('#detail-delete-btn').addEventListener('click', async () => {
    ...
    if (ok) {
      await state.deleteScheduleItem(id);
...
function renderDetailCardHtml(item) {
  ...
  return `
    <div class="cal-detail-card" data-id="${id}">
      ...
      <div class="cal-detail-card__row">
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
        ${formatTime(item.startTime)} - ${formatTime(item.endTime)}
      </div>
      ${item.lecturer ? `<div class="cal-detail-card__row">${escapeHtml(item.lecturer)}</div>` : ''}
      <button type="button" class="btn btn-primary btn-sm cal-detail-card__btn" data-id="${id}">Lihat Detail</button>
    </div>
  `;
}
...
function bindScheduleEvents(container) {
  ...
  container.querySelectorAll('.delete-schedule').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (ok) {
        await state.deleteScheduleItem(btn.dataset.id);
        showToast('Kelas dihapus', 'success');
        renderSchedule(container);
      }
    });
...
function showScheduleModal(item = null) {
  ...
  modal.querySelector('#save-schedule').addEventListener('click', async () => {
    ...
    try {
      if (isEdit) {
        await state.updateScheduleItem(itemId, payload);
        showToast('Kelas diperbarui', 'success');
      } else {
        await state.addScheduleItem(payload);
