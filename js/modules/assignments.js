...
function renderAssignments(container) {
  ...
  const statusSelect = createCustomSelect(statusOptions, {
  ...
  container.querySelector('#assign-status-container').appendChild(statusSelect);

  const priorityOptions = [
    { value: 'all', label: 'Semua Prioritas' },
    ...PRIORITIES.map(p => ({ value: p, label: tPriority(p) }))
  ];
  const prioritySelect = createCustomSelect(priorityOptions, {
  ...
  container.querySelector('#assign-priority-container').appendChild(prioritySelect);

  const sortOptions = [
    { value: 'deadline', label: 'Urutkan Tenggat' },
    { value: 'priority', label: 'Urutkan Prioritas' },
    { value: 'title', label: 'Urutkan Judul' }
  ];
  const sortSelect = createCustomSelect(sortOptions, {
    value: filters.sort,
    placeholder: 'Urutkan Tenggat',
    onChange: (val) => {
      filters.sort = val;
      renderAssignments(container);
    }
...
function showAssignmentModal(assignment = null) {
  ...
  modal.querySelector('#save-assignment').addEventListener('click', async () => {
    ...
    try {
      if (isEdit) {
        await state.updateAssignment(assignmentId, payload);
        showToast('Tugas diperbarui', 'success');
      } else {
        await state.addAssignment(payload);
